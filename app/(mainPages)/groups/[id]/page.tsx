...
function Page(props: cardProps) {
  ...
  async function handleAcceptApprove(chatId: string, userId: string) {
    console.log(`Approve ${chatId} : ${userId}`);

    const resp = await acceptUserAprroval(userId, chatId);
  }

  async function handleDenyApprove(chatId: string, userId: string) {
    const resp = await denyUserAprroval(userId, chatId);
  }

  //   console.log("Joined users:", chatData.joined);
      ...
      <div className="bg-primary_colors-black_1 flex justify-between py-2 px-3 rounded-sm gap-2 items-center">
        ...
        <div className="flex gap-4">
          ...
          <Drawer>
            ...
            <DrawerContent className="bg-primary_colors-black_2 border-primary_colors-black_2">
              <DrawerHeader>
                <DrawerTitle>Approve List</DrawerTitle>
                <DrawerClose />
              </DrawerHeader>
              <DrawerDescription className="px-4 mb-10 text-primary_colors-white_1">
                <div className="flex flex-col gap-2">
                  {chatData.approveList ? (
                    chatData.approveList?.map((user, index) => (
                      <div key={index} className="flex items-center gap-4">
                    ...
                    <p className="font-mono">No Approve Requests Found</p>
                  )}
                </div>
              </DrawerDescription>
            </DrawerContent>
          ...
          <div className="flex flex-col gap-3 flex-grow self-stretch px-4 overflow-auto">
            {chatData?.messages?.map((message: MessageType, index: number) => (
              <div
                key={message.id}
                ref={
                  index === chatData.messages.length - 1 ? lastMessageRef : null
                }
                className={`flex ${
